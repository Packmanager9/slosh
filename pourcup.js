...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class SpringOP {
        constructor(body, anchor, length) {
                this.body = body
                this.anchor = anchor
                    this.beam = new LineOP(body, anchor, "#00AAFF80", 4.9)
                this.length = length
            this.gravity = 0
        }
        balance() {
        ...
        clean(){
            if (this.beam.hypotenuse() > this.length*2) { //this.length*5.5
    ...
    class Water{
        constructor(x,y){
            this.body = new Circle(x,y, 5, "#00AAFF80", 0, 1, 1)
        ...
        draw(){
            ...
            this.body.repel()
            this.body.radius = (this.links.length*.5)+4
            this.bigbody = new Circle(this.body.x,this.body.y, 15, "#00AAFF80", 0, 1, .99)
    ...
    class Cup{
        constructor(){
            this.center = new Circle(350,350, 1, "red")
            this.points = []
            this.beams = []
            ...
            this.drops = []
            for(let t = 0;t<400;t++){
                let water = new Water(180+Math.random()*280, 180+Math.random()*250)
        ...
        draw(){
            ...
            control(this.center, 6)
            this.beams = []
            this.points = []
            this.angle= this.anglesto
            for(let t = 0;t<22;t++){
                let point = new Circle(this.center.x+(Math.cos(this.angle)*230),  this.center.y+(Math.sin(this.angle)*230), 2, "blue")
                this.points.push(point)
                this.angle+=(Math.PI/16)
            }
            for(let t = 1;t<this.points.length;t++){
                this.beams.push(castBetween(this.points[t], this.points[t-1], 10, 10))
            }
            for(let t = 1;t<this.points.length;t++){
                let link = new LineOP(this.points[t], this.points[t-1], "white", 10)
                link.draw()
            }
            this.shape = new Shape(this.beams)

            for(let t = 0;t<this.drops.length;t++){
                if(this.shape.doesPerimeterTouch(this.drops[t].body)){
                    // let link = new LineOP(this.center, this.drops[t].body)
                    // if(link.hypotenuse()<181){
                        this.drops[t].body.xmom += (this.center.x-this.drops[t].body.x)/20
                        this.drops[t].body.ymom += (this.center.y-this.drops[t].body.y)/20
                        if(this.drops[t].body.ymom > 0){
                            this.drops[t].ymom*=-1
                        }
                    // }else{
                    //     this.drops[t].body.xmom -= (this.center.x-this.drops[t].body.x)/40
                    //     this.drops[t].body.ymom -= (this.center.y-this.drops[t].body.y)/40
                    // }
                }
                this.drops[t].draw()
            }
                ...
                for(let k = 0;k<this.drops.length;k++){
                    if(t!=k){
                        if(this.drops[t].bigbody.doesPerimeterTouch(this.drops[k].bigbody)){
                            this.drops[t].body.xmom -= (this.drops[k].body.x-this.drops[t].body.x)/50
                            this.drops[t].body.ymom -= (this.drops[k].body.y-this.drops[t].body.y)/50
                            if(!this.drops[t].links.includes(k) && !this.drops[k].links.includes(t) && this.drops[t].links.length<6 &&  this.drops[k].links.length<6){
                                let link = new SpringOP(this.drops[t].body, this.drops[k].body, 16)
            ...
            for(let t = 0;t<this.drops.length;t++){
                for(let k = 0;k<this.drops.length;k++){
                    if(t!=k){
                        ...
                        this.drops[t].body.xrepel += (Math.cos(angleRadians) * distance) / 2
                        this.drops[t].body.yrepel += (Math.sin(angleRadians) * distance) / 2
                        this.drops[t].body.xmom += (Math.cos(angleRadians) * distance) / 1
                        this.drops[t].body.ymom += (Math.sin(angleRadians) * distance) / 1
                        this.drops[k].body.xrepel -= (Math.cos(angleRadians) * distance) / 2
                        this.drops[k].body.yrepel -= (Math.sin(angleRadians) * distance) / 2
                        this.drops[k].body.xmom -= (Math.cos(angleRadians) * distance) / 1
                        this.drops[k].body.ymom -= (Math.sin(angleRadians) * distance) / 1
